	...
	for communityMember in communityResult:
		if (communityMember.IsNIdIn(currentUser + 1)):  # current user is in this community
			...
			for nodeMember in communityMember:
				if (matrixScore[nodeMember - 1][targetMovie] != 0 and nodeMember - 1 != currentUser):
					totalCnt = totalCnt + 1
			...
			if (totalCnt == 0):
	    		print "totalCnt = 0 and community size", communityMember.Len() 
...
def predictionBenchMark_1(currentUser, targetMovie):
	...
	cumulatedScore = 0.0
	simuSum = 0.0
	for user in range(0, matrixSimlarityUser.shape[0]):
		if (user == currentUser):
			continue
		elif (matrixScore[user][targetMovie] != 0):
...
def predictionMethod_2(currentUser, targetMovie, communityResult):
	...
	for communityMember in communityResult:
		if (communityMember.IsNIdIn(targetMovie + 1)):  # current user is in this community
		    for nodeMember in communityMember:
		    	if (matrixScore[currentUser][nodeMember - 1] != 0 and nodeMember - 1 != targetMovie):
		    		totalCnt = totalCnt + 1
...
def predictionBenchMark_2(currentUser, targetMovie):
	...
	cumulatedScore = 0.0
	simuSum = 0.0
	for movie in range(0, matrixSimlarityUser.shape[1]):
		if (movie == targetMovie):
			continue
		elif (matrixScore[currentUser][movie] != 0):
		...
		for y in range(yStart, yEnd):
			...
			if (matrixScore[x][y] != 0):
				...
				if (flag == 1):
					...
					#print predictedScore, predictedScore2, goldScore
				elif (flag == 4):
				...
				if (flag == 1):
					print predictedScore, goldScore
				count += 1.0
	if (count == 0):
		return 0
	else:
		print "Valid Data Ratio: ", count * 1.0 / ((xEnd - xStart) * (yEnd - yStart))
...
def main():
	...
	for com in MGcommunityResult:
		print len(com)
	# xStart Index of systemTest start from 1.
	print systemTest(1, 500, 1, 550, UGcommunityResult, 1)
	# print systemTest(1, 500, 1, 550, UGcommunityResult, 3)
